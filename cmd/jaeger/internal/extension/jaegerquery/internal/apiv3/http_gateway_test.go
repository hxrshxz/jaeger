...
func TestHTTPGatewayFindTracesLimitAndNumTraces(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name: "should use limit=0 when provided and ignore num_traces",
			query: func() url.Values {
				q := newBaseQuery()
				q.Set(paramLimit, "0")
				q.Set(paramNumTraces, "75")
...
```I will use `run_shell_command` with `sed` to modify the file. First, I will replace the `TestRegisterHTTPGateway` function.```go
// Copyright (c) 2023 The Jaeger Authors.
// SPDX-License-Identifier: Apache-2.0
